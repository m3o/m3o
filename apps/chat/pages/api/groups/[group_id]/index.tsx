...
//         } catch ({ error, code }) {
//             console.error(
//                 `Error loading recent messages: ${error}, code: ${code}`
//             )
...
//                 } catch ({ error, code }) {
//                     console.error(
//                         `Error loading messages: ${error}, code: ${code}`
//                     )
//                 }
//             })
//     )
...
export default authenticatedHandler(async function handler(req, res, user) {
    try {
        const { records = [] } = await m3o.db.read({
            ...
            query: `groupId == "${req.query.group_id}" and userId == "${user.id}"`,
        })

        if (records.length === 0) {
            res.status(403).send({
                message: 'You are unable to view this group',
            })
        }

        const { records: groupRecords = [] } = await m3o.db.read({
            table: tableNames.groups,
            query: `id == "${req.query.group_id}"`,
        })

        res.json({ group: groupRecords[0] })
    } catch (error) {
        console.log(error)
    }
})
